...
- **LLM providers**: `ProviderRegistry` over Ollama (local, auto-starts daemon), OpenRouter, OpenAI, NineRouter, custom OpenAI-compatible. Free Claude Code (FCC) integration adapter at `/api/fcc/v1/messages` (powered by `alishahryar1/free-claude-code`) routes Claude Code in browser.
