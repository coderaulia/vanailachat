...
- **LLM providers**: `ProviderRegistry` over Ollama (local, auto-starts daemon), OpenRouter, OpenAI, NineRouter, custom OpenAI-compatible, plus the DeepSeek Harness adapter (`src/backend/services/deepseekHarness.ts` powered by `deepseek-ai/deepseek-harness`).
